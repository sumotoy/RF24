 ...
 *modified by max mc costa (sumotoy) for teensy3
 */
...
void RF24::print_status(uint8_t status){
	...
	Serial.print((status & _BV(TX_FULL))?1:0);
	Serial.print("\r\n");
	#else
...
void RF24::print_observe_tx(uint8_t value){
	...
	Serial.print((value >> ARC_CNT) & B1111);
	Serial.print("\r\n");
	#else
...
/****************************************************************************/
	...
	static const char * const rf24_pa_dbm_e_str_P[] PROGMEM = {rf24_pa_dbm_e_str_0,rf24_pa_dbm_e_str_1,rf24_pa_dbm_e_str_2,rf24_pa_dbm_e_str_3};
#endif
...
void RF24::begin(void){
  ...
  // reset our data rate back to default value. This works because a non-P variant won't allow the data rate to be set to 250Kbps.
...
bool RF24::write(const void* buf, uint8_t len, const bool multicast){
  ...
  // or MAX_RT (maximum retries, transmission failed).  Also, we'll timeout in case the radio is flaky and we get neither.

  // IN the end, the send should be blocking.  It comes back in 60ms worst case. Generally much faster.
	uint8_t observe_tx;
	uint8_t status;
...
bool RF24::available(uint8_t* pipe_num){
  ...
  //IF_SERIAL_DEBUG(print_status(status));
    ...
    // If the caller wants the pipe number, include that
		...
		if (status & _BV(TX_DS)) write_register(STATUS,_BV(TX_DS));
	}
	return result;
}
...
void RF24::whatHappened(bool& tx_ok,bool& tx_fail,bool& rx_ready){
  // Read the status & reset the status in one easy call Or is that such a good idea?
...
void RF24::openReadingPipe(uint8_t child, uint64_t address){
  // If this is pipe 0, cache the address.  This is needed because openWritingPipe() will overwrite the pipe 0 address, so startListening() will have to restore it.
    ...
    // Note it would be more efficient to set all of the bits for all open pipes at once.  However, I thought it would make the calling code more simple to do it this way.
	#if defined(TEENSY3X)
